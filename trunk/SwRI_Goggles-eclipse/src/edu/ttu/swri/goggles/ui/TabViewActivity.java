...
public class TabViewActivity extends TabActivity {
	...
	private Type typeOfElementMates = new TypeToken<ArrayList<ElementMate>>() {
	}.getType();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tabHost = getTabHost();
		tabHost.addTab(tabHost.newTabSpec("Friendlies")
				.setIndicator("Friendlies").setContent(R.id.layout_tab_friends));
		tabHost.addTab(tabHost.newTabSpec("Notifications")
				.setIndicator("Notifications")
				.setContent(R.id.layout_tab_messages));
		tabHost.addTab(tabHost.newTabSpec("Markers").setIndicator("Markers")
				.setContent(R.id.layout_tab_pois));
	...
	public void startStopTracking() {
		startService(new Intent(this,
				edu.ttu.swri.messenger.FriendUpdateService.class));
	...
	private void setUpdatedFriendLocationText() {
		try {
			ElementMate friendLocation = AppContext
					.getCurrentUserFriendLatestLocation();
			ElementMate myLocation = AppContext.getCurrentUserLatestLocation();

			if (friendLocation == null || myLocation == null
					|| friendLocation.getLatitude() == 0
					|| friendLocation.getLongitude() == 0
			...
			Location.distanceBetween(AppContext.getCurrentUserLatestLocation()
					.getLatitude(), AppContext.getCurrentUserLatestLocation()
					.getLongitude(), friendLocation.getLatitude(),
	...
	public class SendMessagesAsync extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... aurl) {
			...
			try {
				...
				// each user has their own queue, when we send message we send
				// it with the user identifier (in this case just name like
				// "Alex")
				// plus the msgTag to identify it's a text message, in this case
				// it's "-im"
				Client client = new Client(AppContext.getProjectId(),
						AppContext.getToken(), Cloud.ironAWSUSEast);
		...
		protected void onPostExecute(String unused) {
			Toast.makeText(getBaseContext(),
					"Message Sent to " + AppContext.getCurrrentUserFriend(),
	...
	public class InnerFriendUpdateReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(
					(AppContext.getFriendLocationAction()))
					|| intent.getAction().equals(
							(AppContext.getUserLocationAction()))) {
				setUpdatedFriendLocationText();
				saveRelativeLocation();
			} else if (intent.getAction().equals(
					(AppContext.getFriendMessageAction()))) {
				ElementMessage msg = gson.fromJson(intent.getExtras()
						.getString(AppContext.getMessageBodyExtra()),
						ElementMessage.class);
				Calendar cal = Calendar.getInstance();
				cal.setTimeInMillis(Long.valueOf(msg.getId()));
				Toast.makeText(getApplicationContext(),
						"New Message From " + msg.getFrom(), Toast.LENGTH_SHORT)
						.show();
			}
		}
	...
	public void populateFriendlies() {
		ArrayList<ElementMate> friends = (ArrayList<ElementMate>) userPrefsDao
				.getElementMates();
		...
		ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_list_item_1, friendlies);
		...
		lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				AppContext.setCurrentUserFriend(lv.getItemAtPosition(position)
						.toString());
				startStopTracking();
		...
		lvf.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
					int position, long arg3) {
				// Toast.makeText(getApplicationContext(), "In Messages",
				// Toast.LENGTH_SHORT).show();
			}
		});
		...
		lvf.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				tvMessages.setText("");
				if (!lvf.getSelectedItem().toString().contains("Found")
						&& !lvf.getSelectedItem().toString().contains("Update")) {
					AppContext.dao.getMessagesFrom(lvf.getSelectedItem()
							.toString());
				}
				// Toast.makeText(getApplicationContext(), "In Item Selected",
				// Toast.LENGTH_SHORT).show();
			}
	...
	private void stopLocationUpdateService() {
		stopService(new Intent(this,
				edu.ttu.swri.messenger.location.LocationUpdateService.class));
	}

	private void stopMessageUpdateService() {
		stopService(new Intent(this,
				edu.ttu.swri.messenger.MessageUpdateService.class));
	}
	...
	public class GetFriendsAsync extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... str) {
			...
			try {
				...
				if (queueSize == 0) {
					friends.add(new ElementMate(
							AppContext.getCurrentUserName(), AppContext
									.getCurrentUserName(), "TTU Student", null));
		...
		settings.edit()
				.putString(
						AppContext.getCurrrentUserFriend()
								+ "-relative-location",
						mDistanceToFriendly.getText().toString()).commit();
	}

	public void setRelativeLocation() {
		String relativeLocationText = settings.getString(
				AppContext.getCurrrentUserFriend() + "-relative-location", "");
