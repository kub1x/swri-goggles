...
import edu.ttu.swri.goggles.persistence.impl.SqliteDAO;
...
public class AlejandroSandboxActivity extends Activity {
	...
	public void getMessage(View v) {
		...
		ElementPoi poi = new ElementPoi("958745625", "Library", "Study Area",
				new Location(LocationManager.PASSIVE_PROVIDER), 95874,
				new Date());
		...
		try {
			...
			Toast.makeText(getBaseContext(), "Original id: " + id,
			...
			Toast.makeText(getBaseContext(), "Restored id: " + id,
					Toast.LENGTH_SHORT).show();
		...
		// Toast.makeText(getBaseContext(), "Getting messages from Q",
		// Toast.LENGTH_SHORT).show();
	...
	public class GetMessagesAsync extends AsyncTask<View, String, String> {
		...
		protected String doInBackground(View... views) {
			...
			// these are the calls to the webservice queue to get messages for
			// the current user, need to send projectid, and token
			// there are some dependancies to google json jars, they are
			// included in the project.
			// we may need to extract the url and content that is generated and
			// send it in a way that
		...
		protected void onPostExecute(String string) {
			tv.setText("");
			if (string.isEmpty()) {
				Toast.makeText(getBaseContext(), "Q is empty",
						Toast.LENGTH_SHORT).show();
				return;
			}
			StringBuilder sb = new StringBuilder();
			...
			Gson gson = new Gson();
			// here we use the gson.jar library to inflate the json back into a
			// live object.
			IronIOMessageImpl gm = gson.fromJson(string,
					IronIOMessageImpl.class);
			sb.append(newLine + newLine
					+ "Message Object Data after json is inflated: ");
			...
			sb.append(newLine + "Formatted Date: "
					+ SimpleDateFormat.getDateTimeInstance().format(sentDate));
	...
	public void sendMessage(View v) throws IOException {
		...
		// we can use the gson.jar library to serialize the object in json
		// format
		Gson gson = new Gson();
	...
	public class SendMessagesAsync extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String unused) {
			...
			Toast.makeText(getBaseContext(), "Message Sent", Toast.LENGTH_SHORT)
					.show();
		}
